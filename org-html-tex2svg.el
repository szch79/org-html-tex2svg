...
;;
;; For export backend authors that want to derive from Org HTML export, there's
;; a set of aliases for transcoders and filter functions.
...
(defgroup org-html-tex2svg nil
  "Async LaTeX to SVG conversion for Org HTML export."
  :group 'org-export-html
  :prefix "org-html-tex2svg-")

;; NOTE: this `defvar' should be placed before `org-html-tex2svg-environments',
;; as this is automatically set by its setter
(defvar org-html-tex2svg--env-regexp nil
  "Regexp matching LaTeX environments that need SVG conversion.
Computed from `org-html-tex2svg-environments'.")

(defcustom org-html-tex2svg-environments
  '("tikzpicture" "tikzcd")
...
(defcustom org-html-tex2svg-html-template "<div[ID] style=\"text-align:center\">
  [SVG]
</div>"
...
(defcustom org-html-tex2svg-trigger-function #'org-html-tex2svg--default-trigger
...
(defcustom org-html-tex2svg-scale 1.0
...
(defvar-local org-html-tex2svg--numbering-table nil
...
(defvar-local org-html-tex2svg--async-tasks nil
...
(defun org-html-tex2svg--needs-svg-p (body)
  "Return non-nil if BODY requires SVG conversion.
Matched against environments in `org-html-tex2svg-environments'."
  (and org-html-tex2svg--env-regexp
...
(defun org-html-tex2svg--compute-hash (body preamble number)
...
use to reuse its cache."
  (org-latex-preview--hash
   org-html-tex2svg-process-default
   preamble
   body
   "svg"
   nil
              ...
              (dolist (attr '("width" "height"))
                (when (re-search-forward
                  ...
                  (let* ((quote-char (match-string 1))
                         ...
                         (unit (match-string 3))
                         (scaled (* val org-html-tex2svg-scale)))
                    (replace-match
                     (format "%s=%s%g%s%s"
...
(defun org-html-tex2svg--format-html (svg-content &optional id)
  "Wrap SVG-CONTENT using `org-html-tex2svg-html-template'.
    ...
    (let ((id-attr (if id (format " id=\"%s\"" id) "")))
      (string-replace "[SVG]" svg-content
                      (string-replace "[ID]" id-attr
                                      org-html-tex2svg-html-template)))))

(defun org-html-tex2svg--start-async-generation (tree info)
          ...
          (org-element-map tree '(latex-environment latex-fragment)
            (lambda (el)
              (let* ((body (org-element-property :value el))
                     ...
                     (hash (org-html-tex2svg--compute-hash
                            body preamble number)))
                (when (and (org-html-tex2svg--needs-svg-p body)
...
(defun org-html-tex2svg--latex-to-svg (el info)
...
Return nil otherwise.  INFO is the export communication channel."
  (let ((body (org-element-property :value el)))
    (when (org-html-tex2svg--needs-svg-p body)
      ;; Wait for async tasks if pending
      (when org-html-tex2svg--async-tasks
...
(defun org-html-tex2svg--org-html-latex-a (orig-fun el contents info)
  ...
  (or (and (funcall org-html-tex2svg-trigger-function
                    ...
                    info)
           (org-html-tex2svg--latex-to-svg el info))
...
(defun org-html-tex2svg--check-dependencies ()
...
;;;###autoload
(define-minor-mode org-html-tex2svg-mode
  "Toggle async LaTeX to SVG conversion for Org HTML export.
When enabled, LaTeX environments matching `org-html-tex2svg-environments' are
